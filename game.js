...
/* =========================================================
   カギ扉前ガーディアン 完全差し込み版 v3
   貼る場所:
   - game.js の一番下
   - 以前貼ったガーディアンコードは削除して、これだけ貼る
   ...
   ========================================================= */
(function(){
  if(window.__keyDoorGuardianPatchV3Applied) return;
  window.__keyDoorGuardianPatchV3Applied = true;
  ...
  */
  ...
  /*
    tryDoor も上書き。
    他の処理から tryDoor(d) が呼ばれても同じルールにする。
  */
  tryDoor = function(d){
  ...
  */
  ...
  console.log("key door guardian patch v3 loaded");
})();
/* =========================================================
   カギ扉前ガーディアン 埋まり防止 + 攻撃AI 修正パッチ v3.1
   貼る場所:
   - 前回の「カギ扉前ガーディアン v3」のさらに下
   ...
   ========================================================= */
(function(){
  if(window.__keyDoorGuardianFixV31Applied) return;
  window.__keyDoorGuardianFixV31Applied = true;
  ...
  function moveGuardianToSafeSpot(e,index){
    ...
    */
    ...
    for(const c of candidates){
      ...
      x = clamp(x,42,G.map.width - w - 42);
      y = clamp(y,42,G.map.height - h - 42);
    ...
    /*
      最後の保険。
      どうしても候補が全部ダメなら、マップ中央寄りに逃がす。
    */
    e.x = clamp(G.map.width / 2 - w / 2 + shift,42,G.map.width - w - 42);
    e.y = clamp(G.map.height / 2 - h / 2,42,G.map.height - h - 42);
    e.wake = true;
    e.attackPause = 0;
    e.__guardianFixedPosition = true;
  }
  ...
  function guardianBossAttackAI(){
    ...
    for(const e of G.enemies){
      ...
      /*
        念のため、通常の雑魚AIに加えて少しだけ自力で近づく。
        壁に埋まっていなければ、これで明確に動く。
      */
      if(len > 56){
  ...
  */
  ...
  console.log("key door guardian fix v3.1 loaded");
})();