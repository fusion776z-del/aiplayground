...
function getEnemyPool(theme){
  const pools = {
    grass: ["slime", "fast", "wind_bat"],
    forest: ["slime", "fast", "wind_bat"],
    cave: ["slime", "blue_slime", "crystal_slime", "fast", "wind_bat", "dark_bat"],
    fire: ["slime", "armored_slime", "fast", "red_fast", "wind_bat", "fire_bat"],
    ruin: ["slime", "armored_slime", "fast", "red_fast", "wind_bat", "fire_bat", "dark_bat"],
    star: ["slime", "crystal_slime", "armored_slime", "fast", "void_fast", "wind_bat", "dark_bat"],
    void: ["slime", "armored_slime", "fast", "void_fast", "wind_bat", "fire_bat", "dark_bat"]
  };
...
function generateTerrain(layout, width, height){
  ...
  if(layout === LayoutTypes.LINE){
    ...
    terrain.push({x:rand(120, 260), y:height - 520, w:92, h:74, type:"tree"});
    terrain.push({x:rand(width - 270, width - 160), y:height - 620, w:104, h:80, type:"tree"});
    terrain.push({x:rand(280, width - 280), y:height - 430, w:78, h:56, type:"rock"});
    terrain.push({x:rand(90, 230), y:height - 720, w:74, h:54, type:"rock"});
  }
  if(layout === LayoutTypes.SPLIT){
    ...
    terrain.push({x:250, y:height - 760, w:90, h:60, type:"rock"});
    terrain.push({x:width - 350, y:height - 760, w:90, h:60, type:"rock"});
  ...
  if(layout === LayoutTypes.LOOP){
    ...
    terrain.push({x:Math.floor(width / 2) - 80, y:height - 1030, w:160, h:40, type:"stone"});
    terrain.push({x:rand(120, 240), y:height - 900, w:78, h:56, type:"rock"});
    terrain.push({x:rand(width - 280, width - 160), y:height - 1000, w:82, h:58, type:"rock"});
  }
  if(layout === LayoutTypes.ZIGZAG){
    ...
    terrain.push({x:120, y:height - 460, w:90, h:70, type:"tree"});
    terrain.push({x:width - 220, y:height - 630, w:90, h:70, type:"tree"});
    terrain.push({x:160, y:height - 800, w:80, h:58, type:"rock"});
    terrain.push({x:width - 230, y:height - 970, w:84, h:60, type:"rock"});
  }
  if(layout === LayoutTypes.MAZE){
    ...
    terrain.push({x:150, y:height - 960, w:220, h:40, type:"stone"});
    terrain.push({x:width - 360, y:height - 1120, w:240, h:40, type:"stone"});
    terrain.push({x:width - 180, y:height - 420, w:88, h:62, type:"rock"});
    terrain.push({x:90, y:height - 880, w:94, h:72, type:"tree"});
    terrain.push({x:width - 210, y:height - 1050, w:96, h:76, type:"tree"});
  }
  if(layout === LayoutTypes.CROSS){
    terrain.push({x:Math.floor(width / 2) - 28, y:height - 980, w:56, h:600, type:"stone"});
    terrain.push({x:100, y:height - 720, w:width - 200, h:44, type:"stone"});
    terrain.push({x:160, y:height - 500, w:width - 320, h:40, type:"stone"});
    terrain.push({x:90, y:height - 800, w:96, h:76, type:"tree"});
    terrain.push({x:width - 190, y:height - 800, w:96, h:76, type:"tree"});
    terrain.push({x:Math.floor(width / 2) - 120, y:height - 540, w:80, h:56, type:"rock"});
    terrain.push({x:Math.floor(width / 2) + 60, y:height - 540, w:80, h:56, type:"rock"});
  }
  if(layout === LayoutTypes.ISLANDS){
    terrain.push({x:width - 280, y:height - 520, w:190, h:46, type:"stone"});
    terrain.push({x:120, y:height - 700, w:190, h:46, type:"stone"});
    terrain.push({x:width - 330, y:height - 880, w:230, h:46, type:"stone"});
    terrain.push({x:Math.floor(width / 2) - 110, y:height - 1050, w:220, h:46, type:"stone"});
    terrain.push({x:Math.floor(width / 2) - 170, y:height - 1220, w:340, h:42, type:"stone"});
    terrain.push({x:width - 220, y:height - 650, w:88, h:62, type:"rock"});
    terrain.push({x:150, y:height - 830, w:92, h:74, type:"tree"});
    terrain.push({x:width - 250, y:height - 980, w:96, h:78, type:"tree"});
  }
  if(layout === LayoutTypes.STAR_TEMPLE){
    ...
    terrain.push({x:90, y:height - 690, w:92, h:72, type:"tree"});
    terrain.push({x:width - 190, y:height - 690, w:92, h:72, type:"tree"});
    terrain.push({x:Math.floor(width / 2) - 150, y:height - 430, w:80, h:58, type:"rock"});
    terrain.push({x:Math.floor(width / 2) + 70, y:height - 430, w:80, h:58, type:"rock"});
  }
  if(layout === LayoutTypes.VOID_CASTLE){
    ...
    terrain.push({x:80, y:height - 430, w:96, h:76, type:"tree"});
    terrain.push({x:width - 190, y:height - 430, w:96, h:76, type:"tree"});
...
function generateChests(theme, width, height){
  ...
  return [
    {id:"key", x:rand(Math.floor(width * 0.55), width - 90), y:rand(Math.floor(height * 0.38), Math.floor(height * 0.64)), w:30, h:24, opened:false, item:{type:"key", id:keyId, name:getKeyName(theme)}},
    {id:"coin", x:rand(60, Math.floor(width * 0.45)), y:rand(Math.floor(height * 0.35), Math.floor(height * 0.72)), w:30, h:24, opened:false, item:{type:"coin", amount:rand(180, 620)}},
    {id:"herb", x:rand(Math.floor(width * 0.55), width - 90), y:rand(Math.floor(height * 0.68), height - 210), w:30, h:24, opened:false, item:{type:"item", id:"small_herb", name:"小さな薬草", count:1}}
  ];
}
...
function generateShops(theme, width, height){
  return [{id:"shop", name:getShopName(theme), x:Math.floor(width * 0.66), y:height - 165, w:34, h:34, color:theme === "star" ? "#9df7ff" : theme === "void" ? "#ffd84d" : "#ffb347"}];
}
...
function generateEnemies(theme, width, height, stageId){
  ...
  for(let i = 0; i < count; i++){
    enemies.push({id:pickEnemy(theme, i + rand(0, 5)), x:rand(60, width - 90), y:rand(360, height - 260)});
  }
  return enemies;
}
...
function createStage(id, theme, layout){
  ...
  return {
    ...
    doors:[{id:"boss_door", x:Math.floor(width / 2) - 78, y:270, w:156, h:44, locked:true, requiredItem:keyId, label:getDoorName(theme)}],
    boss:{id:"boss", name:getBossName(theme), x:Math.floor(width / 2), y:165, w:105 + id * 7, h:92 + id * 5, hp:bossHp, maxHp:bossHp, color:getBossColor(theme)},
    bossRoom:{x:42, y:42, w:width - 84, h:228},
    finalFlow: theme === "void" ? {keyToCircle:true, circleToBossRush:true, bossRushToTrueCircle:true, trueCircleToAbyss:true} : null,
    autoBossRush:false,
    requireMagicCircle:true,
    onClear:function(g){
      g.message = getClearMessage(theme);
      if(!g.player.cores.includes(coreId)){ g.player.cores.push(coreId); }
